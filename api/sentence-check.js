export default async function handler(req, res) {
  ...
  const userPrompt = `A Slovak learner was asked to write a sentence using the word "${word}" (or a close synonym).
Their sentence: "${sanitized}"

Check:
1. Does the sentence use "${word}" or a synonym naturally and correctly?
2. Are there any grammar mistakes?

Reply ONLY with JSON: { "ok": true/false, "feedback": "one short sentence" }
- If correct: ok=true, feedback = brief praise + maybe one tip
  ...
  try {
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 150,
        system: 'You are a concise English teacher. Reply only with the JSON object, no other text.',
