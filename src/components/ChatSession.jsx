...
function getStarter(ctx) {
  if (ctx) {
    const task = ctx.taskType === 'writing' ? 'wrote about' : 'spoke about'
    return `Hey! I saw you ${task} this today: "${ctx.prompt}" — great work! Can you tell me more about it?`
  }
      ...
      {todayCtx && (
        <div className="chat-phrases-bar" style={{ background: 'linear-gradient(90deg, #ede9fe, #ddd6fe)', borderColor: '#a78bfa' }}>
          <span className="chat-phrases-label" style={{ color: '#6d28d9' }}>
            {todayCtx.taskType === 'writing' ? '✍️' : '🎤'} Today's topic:
          </span>
